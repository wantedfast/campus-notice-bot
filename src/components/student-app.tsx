...
export default function StudentApp({ initialTab = 'chat' }: { initialTab?: 'chat' | 'notices' }) {
  ...
  const filtered = notices.filter(n => `${noticeLabel(n)}\n${n.body}`.toLowerCase().includes(search.toLowerCase()));
  return <div className="app-shell">
    ...
    <main className="main-panel">
      ...
      {tab === 'chat' ? <>
        ...
        </div></> : <div className="notices-view"><div className="notices-heading"><span className="eyebrow">CAMPUS BULLETIN</span><h1>校园里的事，都在这里。</h1><p className="muted">查看最新安排，也可以回到对话里继续问。</p></div><label className="search-box"><Search size={19}/><input placeholder="搜索通知标题或内容" aria-label="搜索通知" value={search} onChange={e => setSearch(e.target.value)}/>{search && <button aria-label="清空搜索" onClick={() => setSearch('')}><X size={16}/></button>}</label><p className="result-count">共 {filtered.length} 条通知</p>{loading ? <p className="empty-inline">正在加载通知…</p> : loadError ? <p role="alert">{loadError}<button className="text-button" onClick={() => void refresh()}>重试</button></p> : !filtered.length ? <div className="large-empty"><BookOpen size={32}/><h2>{search ? '没有找到相关通知' : '通知栏还是空的'}</h2><p>{search ? '换个关键词试试。' : '管理员发布后，就会显示在这里。'}</p></div> : <div className="notice-cards">{filtered.map(n => <Link className="notice-card" href={`/notices/${n.id}`} key={n.id}><div className="notice-card-top"><span className="notice-tag">校园通知</span><ArrowUpRight size={19}/></div><h2>{noticeLabel(n)}</h2><p>{n.body}</p><span className="time"><Clock3 size={14}/>{dateLabel(n.noticeAt)}</span></Link>)}</div>}</div>}
    </main>
